return {
    {
        ...
        config = function()
            require("code_runner").setup({
                ...
                filetype = {
                    java = "echo -e '\\033[1;34m[System]\\033[0m Compiling and Running Java...' && root=$(git rev-parse --show-toplevel 2>/dev/null || pwd) && cd $root && if [ -f pom.xml ]; then mvn compile exec:java -Dexec.mainClass=$(echo $file | sed 's|.*src/main/java/||' | sed 's|\\.java$||' | tr '/' '.'); else javac -d bin -sourcepath src $file && java -cp 'bin:lib/*' $(echo $file | sed \"s|$root/src/||\" | sed \"s|\\.java$||\" | tr '/' '.'); fi",
